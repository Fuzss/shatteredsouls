...
public class ShatterRenderHandler {
    private static final Map<LivingEntityRenderer<?, ?, ?>, EntityModelAdapter> MODEL_ADAPTERS = new MapMaker().concurrencyLevel(
            1).weakKeys().makeMap();
    private static final ContextKey<Unit> SHATTER_ANIMATION_RENDER_PROPERTY_KEY = new ContextKey<>(ShatteredSouls.id(
            "shatter_animation"));
    public static final ContextKey<Vec3> DELTA_MOVEMENT_RENDER_PROPERTY_KEY = new ContextKey<>(ShatteredSouls.id(
            "delta_movement"));
    public static final ContextKey<Integer> ENTITY_ID_RENDER_PROPERTY_KEY = new ContextKey<>(ShatteredSouls.id(
            "entity_id"));
    public static final ContextKey<Float> BODY_ROT_RENDER_PROPERTY_KEY = new ContextKey<>(ShatteredSouls.id("body_rot"));

    public static void onExtractRenderState(Entity entity, EntityRenderState entityRenderState, float partialTick) {
        if (entity instanceof LivingEntity livingEntity && livingEntity.isDeadOrDying() && !ShatteredSouls.CONFIG.get(
                ClientConfig.class).shatterAnimationBlacklist.contains(entity.getType())) {
            ...
            RenderStateExtraData.set(entityRenderState, SHATTER_ANIMATION_RENDER_PROPERTY_KEY, Unit.INSTANCE);
            ...
            RenderStateExtraData.set(entityRenderState, ENTITY_ID_RENDER_PROPERTY_KEY, livingEntity.getId());
            // do not lerp this value to prevent flickering
            RenderStateExtraData.set(entityRenderState, BODY_ROT_RENDER_PROPERTY_KEY, livingEntity.yBodyRotO);
        }
    }

    @SuppressWarnings("unchecked")
    public static <T extends LivingEntity, S extends LivingEntityRenderState, M extends EntityModel<? super S>> EventResult onBeforeRenderEntity(S renderState, LivingEntityRenderer<T, S, M> entityRenderer, float partialTick, PoseStack poseStack, SubmitNodeCollector submitNodeCollector) {
        if (!RenderStateExtraData.has(renderState, SHATTER_ANIMATION_RENDER_PROPERTY_KEY)) {
        ...
        ((LivingEntityRendererAccessor<T, S, M>) entityRenderer).shatteredsouls$callScale(renderState, poseStack);
        poseStack.translate(0F, -1.501F, 0F);
        poseStack.mulPose(Axis.YP.rotationDegrees(RenderStateExtraData.getOrDefault(renderState,
        ...
        modelPartRenderer.setupModelParts(renderState, partialTick);

        RenderType renderType = RenderType.entityTranslucent(entityRenderer.getTextureLocation(renderState));
        // vanilla seems to stop rendering for very low alpha, so we cannot fade out properly
        float animationProgress = Mth.clamp(
                (renderState.deathTime + partialTick) / ShatterTickHandler.SHATTER_DEATH_TIME, 0.0F, 1.0F);
        // TODO re-enable this lol
//        modelPartRenderer.setupAndRenderModelParts(renderState,
//                poseStack,
